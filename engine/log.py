...
_confmap_dict = {
    'level': 'GLOG_v',
    'console': 'GLOG_logtostderr',
    'filepath': 'GLOG_log_dir',
...
def _adapt_cfg(kwargs):
    ...
    """
    kwargs['level'] = _gloglevel_to_name.get(
        kwargs.get('level', _logger_def_level))
    kwargs['stderr_level'] = _gloglevel_to_name.get(
        kwargs.get('stderr_level', _logger_def_level))
    kwargs['console'] = not kwargs.get('console') == _std_off
    kwargs['maxBytes'] = int(kwargs.get('maxBytes', _logger_def_max_bytes))
    kwargs['backupCount'] = int(
        kwargs.get('backupCount', _logger_def_backup_count))
    return kwargs
...
def get_level():
    ...
    level_to_glog_level = dict(
        zip(_name_to_level.values(), _gloglevel_to_name.keys()))
...
def _verify_config(kwargs):
    ...
    if console is not None:
        if not console.isdigit() or console not in (_std_off, _std_on):
            raise ValueError(
                f'Incorrect value, The value of {_confmap_dict["console"]} must be 0 or 1,'
                f' Output log to console, configure to 1.')

        if console == _std_off and not file_path:
            raise ValueError(
                f'When {_confmap_dict["console"]} is set to 0, The directory of '
        ...
        if console == _std_off and max_bytes is not None:
            if not max_bytes.isdigit():
                raise ValueError(
                    f'Incorrect value, The value of {_confmap_dict["maxBytes"]} must be positive integer. '
        ...
        if console == _std_off and backup_count is not None:
            if not backup_count.isdigit():
                raise ValueError(
                    f'Incorrect value, The value of {_confmap_dict["backupCount"]} must be positive '
...
def _verify_level(level):
    ...
    if level_name not in _name_to_level:
        raise ValueError(
            f'Incorrect log level:{level}, Please check the configuration of GLOG_v or '
...
def get_log_config():
    ...
    if handler.name == 'FileHandler':
        ...
        level_to_glog_level = dict(
            zip(_name_to_level.values(), _gloglevel_to_name.keys()))
        config_dict['GLOG_stderrthreshold'] = level_to_glog_level.get(
    ...
    try:
        ...
        # Set rotatingFileHandler for the file appender
            ...
            console_handler.setLevel(
                kwargs.get('stderr_level', logging.WARNING))
            logger.addHandler(console_handler)
